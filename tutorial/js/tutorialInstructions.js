...
// Adjust max view based on the condition
var max_view=30;
if(testType=="1"){
    max_view=14;
}else if(testType=="2"){
...
// Set the page number
d3.select('#tutorial_page_number').text("" + displayed_view + "/"+ max_view);
...
if(current_view==5){ // Scatter plot panel
	...
	d3.select("#tutorial_header").text("Target designs")
	d3.select("#tutorial_text_1").html('<p>For each task, a group of dots will be highlighted in a light blue color. '
	...
	d3.select("#tutorial_header").text("Number of designs")
	d3.select("#tutorial_text_1").html('<p>The total number of designs and the number of target designs are displayed in the boxes '
	...
	d3.select("#tutorial_header").text("Design Inspection")
	d3.select("#tutorial_text_1").html('<p>If you hover your mouse over a design on the scatter plot, the relevant information '
    ...
    d3.select('#tutorial_header').text('Finding "good" features');
    d3.select('#tutorial_text_1').html('<p>Now, we will define what a good feature is. Your goal in this experiment is to find “good” features that explain the target region well. Below are some examples of how features might look like (these are just examples, so you don\'t have to pay attention to the details): </p>'
    ...
    d3.select('#tutorial_header').text('Coverage of target designs');
