...
function adjust_logical_connective(){
    ...
    if(second_feature[0][0]==null){
        ...
        logical_connective_div.select('select').on("change",function(d){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(current_view==19){
        ...
        var filter_level = [];
        var filter_logic_indent_levels = [];
        ...
        application_status.selectAll('.applied_feature')[0].forEach(function(d){
            ...
            if(d3.select(d).select('.feature_application_logical_connective')[0][0]){
                logic = d3.select(d).select('.feature_application_logical_connective')[0][0].value; 
                logic_indent_level = +d3.select(d).select('.logical_connective').attr('level');
            }else{
                logic = '&&';
                logic_indent_level = level;
            }
            ...
            if(activated){
                ...
                filter_level.push(level);
                filter_logic_indent_levels.push(logic_indent_level);
            }
        });
        
        var check_logic = false;
        var check_indentation = false;
        
        if(filter_logical_connective.length==3){
            if(filter_logical_connective[1]=='&&' && filter_logical_connective[2]=='||'){
                if(filter_level[0] < filter_level[1] && filter_level[1] == filter_level[2]){
                    if(filter_logic_indent_levels[2] == filter_level[2]){
                        activate_continue_button();
                        if(max_view_reached < 19){max_view_reached = 19;}
                   }
                }   
            }
        }
